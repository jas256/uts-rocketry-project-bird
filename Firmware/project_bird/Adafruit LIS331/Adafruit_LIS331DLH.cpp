...
/*!
 *  @brief  Instantiates a new LIS331DLH class in I2C
 */
Adafruit_LIS331DLH::Adafruit_LIS331DLH(){};
 ...
 */
bool Adafruit_LIS331DLH::begin_I2C(uint8_t i2c_address, TwoWire *wire,
                                  int32_t sensor_id) {
  if (i2c_dev) {
 ...
 *    @return True if initialization was successful, otherwise false.
 */
bool Adafruit_LIS331DLH::begin_SPI(uint8_t cs_pin, SPIClass *theSPI,
                                  int32_t sensor_id) {
  i2c_dev = NULL;
 ...
 *    @return True if initialization was successful, otherwise false.
 */
bool Adafruit_LIS331DLH::begin_SPI(int8_t cs_pin, int8_t sck_pin,
 ...
 */
bool Adafruit_LIS331DLH::_init(int32_t sensor_id) {
  ...
  setRange(LIS331DLH_RANGE_8_G);

  return true;
}

void Adafruit_LIS331DLH::_scaleValues(void) {
  ...
  if (range == LIS331DLH_RANGE_2_G)
    scale_max = 2;
  if (range == LIS331DLH_RANGE_4_G)
    scale_max = 4;
  if (range == LIS331DLH_RANGE_8_G)
    scale_max = 8;
